package com.coolappstore.everhaptics.by.svhp.ui.screens.navhaptics
...
fun NavBarHapticsScreen(
    ...
    Scaffold(
        ...
        topBar = {
            LargeTopAppBar(
                title = { Text(text = stringResource(R.string.navbar_haptics_title), style = MaterialTheme.typography.displaySmall) },
                navigationIcon = {
        ...
        LazyColumn(
            ...
            item(key = "navbar_section") {
                SectionCard(title = stringResource(R.string.navbar_haptics_section), icon = Icons.Rounded.Home) {
                    ...
                    HorizontalDivider(color = MaterialTheme.colorScheme.outlineVariant, thickness = 0.5.dp, modifier = Modifier.padding(horizontal = 20.dp))
                    PatternSelector(selected = settings.navBarHapticPattern, onPatternSelected = onPatternSelected)
                }
            }

            item(key = "navbar_test") {
                HapticTestButton(label = stringResource(R.string.navbar_haptics_test), enabled = settings.navBarHapticEnabled, onClick = onTestHaptic)
            }
